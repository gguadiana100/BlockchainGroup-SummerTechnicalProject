...
contract('NFTMarketplace', (accounts) => {
  ...
  describe('start testing', async () => {
    ...
    it('checking Lock NFT function three tokens', async () =>{
      ...
      await nft_contract.approve(market_contract.address,3,{from:accounts[2]})
      await market_contract.lockNFT(nft_contract.address,3,{from:accounts[2]})
      ...
      const owner_3 = await nft_contract.ownerOf(3)
      assert.equal(owner_3,market_contract.address)
      ...
      await market_contract.unlockNFT(nft_contract.address,3,accounts[2])
      const ownerNew_3 = await nft_contract.ownerOf(3)
      assert.equal(ownerNew_3,accounts[2])
  })
